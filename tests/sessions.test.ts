...
import { claude, QueryBuilder, Session } from '../src/fluent.js';
...
describe('Session Management', () => {
  ...
  describe('withSession method', () => {
    it('should create a session builder that maintains session context', async () => {
      mockQuery
        .mockImplementationOnce(async function* () {
          yield {
            type: 'system',
            subtype: 'session',
            data: { session_id: 'auto-session-abc' }
          };
          yield {
            type: 'assistant',
            content: [{ type: 'text', text: 'Ace of Spades' }]
          };
          yield { type: 'result', content: 'done' };
        })
        .mockImplementationOnce(async function* () {
          yield {
            type: 'assistant',
            content: [{ type: 'text', text: 'You picked the Ace of Spades' }]
          };
          yield { type: 'result', content: 'done' };
        });
      ...
      expect(firstCard).toBe('Ace of Spades');
      expect(secondCard).toBe('You picked the Ace of Spades');

      // Both queries should be made with the same session
      expect(mockQuery).toHaveBeenCalledTimes(2);
      expect(mockQuery).toHaveBeenNthCalledWith(
        1,
        'Tell me a random card from the deck',
        expect.objectContaining({
          model: 'sonnet',
          permissionMode: 'bypassPermissions'
        })
      ...
      expect(mockQuery).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          sessionId: 'auto-session-abc'
        })
      );
    });

    it('should return a session builder that inherits parent configuration', () => {
      mockQuery.mockImplementation(async function* () {
        yield { type: 'result', content: 'done' };
      });

      const builder = claude().withModel('opus').withTimeout(10000).debug(true);

      const session = builder.withSession();

      // Session should inherit parent config
      session.query('test');
    ...
    it('should allow multiple queries within the same session', async () => {
      mockQuery
        .mockImplementationOnce(async function* () {
          yield {
            type: 'system',
            subtype: 'session',
            data: { session_id: 'multi-session-xyz' }
          };
          yield {
            type: 'assistant',
        ...
        .mockImplementationOnce(async function* () {
          yield {
            type: 'assistant',
            content: [{ type: 'text', text: 'About 2.1 million people' }]
          };
          yield { type: 'result', content: 'done' };
        });
      ...
      const capital = await session.query('What is the capital of France?').asText();
      const country = await session.query('Which country is that in?').asText();
      const population = await session.query('What is the population?').asText();

      expect(capital).toBe('Paris');
      expect(country).toBe('France');
      expect(population).toBe('About 2.1 million people');

      // All queries after the first should include session ID
      expect(mockQuery).toHaveBeenCalledTimes(3);
      expect(mockQuery).toHaveBeenNthCalledWith(
        2,
        'Which country is that in?',
        expect.objectContaining({
          sessionId: 'multi-session-xyz'
        })
      );
      expect(mockQuery).toHaveBeenNthCalledWith(
        3,
